...
import { ArrowLeft, Clock, CheckCircle, AlertCircle, Download, Search } from 'lucide-react';
...
export default function CitizenTrackStatus() {
  ...
  const certificateFileName = useMemo(() => {
    if (!application) return 'certificate.txt';
    return `${application.application_number}-certificate.txt`;
  }, [application]);

  const downloadCertificate = () => {
    if (!application) return;
    const text = [
      'SEVA PORTAL — CERTIFICATE (MOCK)',
      '',
      `Application ID: ${application.application_number}`,
      `Status: ${application.status.toUpperCase()}`,
      `Issued on: ${new Date().toLocaleString('en-IN')}`,
      '',
      'This is a mock downloadable certificate file generated by the portal UI.',
    ].join('\n');
            ...
            {application.status === 'rejected' && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                ...
                <p className="text-sm text-red-700 mt-1">
                  ...
                  {(application.remarks && application.remarks.trim().length > 0) ? application.remarks : 'No remarks provided by officer.'}
                </p>
              </div>
            )}

            {application.status !== 'rejected' && application.remarks && application.remarks.trim().length > 0 && (
              <div className="bg-slate-50 border border-slate-200 rounded-lg p-4">
                <p className="text-sm font-semibold text-slate-800">Officer Remarks</p>
                <p className="text-sm text-slate-700 mt-1">{application.remarks}</p>
              </div>
            )}
            ...
            {application.status === 'approved' && (
              <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-emerald-700 mt-1">You can download your certificate.</p>
                </div>
                <button
                  type="button"
